...
module PomPomPom
  class Dependency < ImmutableStruct.new(:group_id, :artifact_id, :version, :packaging, :classifier, :optional, :exclusions)
    ...
    def self.parse(artifact_coordinates)
      raise %(Malformed artifact coordinate: "#{artifact_coordinates}") if artifact_coordinates.nil? || artifact_coordinates.strip.length == 0
      components = artifact_coordinates.strip.split(':')
      raise %(Malformed artifact coordinate: "#{artifact_coordinates}") if components.size < 3 || components.size > 5
